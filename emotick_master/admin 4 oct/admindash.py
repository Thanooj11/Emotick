...
def submit():
     ...
     if start_date!='' and end_date!='':
         ...
         for i in range(len(row)):
             ...
             cur.execute("select * from emolog where emolog.id in (select id from device_category where cid = %s ) and type_of_room like %s and (timestamp >=%s and timestamp < %s)",((cid),(room)+"%",(start_date),(end_date),))
...
def indexResult():
   ...
   return render_template('index1.html', happy = happyq1, sad = sadq1, okay = okayq1)
...
def index():
   ...
   return render_template('index1.html', happy = happy, sad = sad, okay = okay , happyq1 = happyq1)
...
if __name__ == '__main__':
    ...
    app.run(debug=True, threaded=True, port= 5010, host='176.23.8.165')
